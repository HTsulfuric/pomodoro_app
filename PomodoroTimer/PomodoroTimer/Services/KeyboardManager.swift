...
class KeyboardManager {
    ...
    @MainActor private func handleSketchyBarSettingsKey(_ event: NSEvent, viewModel _: AppCoordinator) -> Bool {
        guard !event.modifierFlags.contains([.command, .control, .option, .shift]) else {
            return false // Don't handle modified keys
        }
    ...
    private func handleCarbonHotkey(event _: EventRef) -> OSStatus {
